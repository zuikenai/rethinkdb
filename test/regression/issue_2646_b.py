    ...
    def run(self):
        ...
        try:
            while not self.stopping:
                ...
                r.table("test").insert([{}]*self.batch).run(conn2, no_reply = True)
                self.counter += 1
                # time.sleep(0.03)
        except Exception, e:
...
with driver.Metacluster() as metacluster:
    ...
    files = driver.Files(metacluster, db_path = "/tmp/tim_2646_db", log_path = "create-output",
    ...
    for i in xrange(3):
        ...
        with ConcurrentWriter(process, num_rows, 100):
