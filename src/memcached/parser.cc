...
void do_one_get(txt_memcached_handler_t *rh, bool with_cas, get_t *gets, int i, order_token_t token) {
    try {
        if (with_cas) {
            ...
            gets[i].res = checked_boost_get<get_result_t>(response.result);
        } else {
...
void do_rget(txt_memcached_handler_t *rh, pipeliner_t *pipeliner, order_source_t *order_source, int argc, char **argv) {
    ...
    try {
        ...
        while (max_items > 0) {
            ...
            rget_result_t results = checked_boost_get<rget_result_t>(response.result);
...
void run_storage_command(txt_memcached_handler_t *rh,
    ...
    if (sc != append_command && sc != prepend_command) {
        ...
        try {
            ...
            res = checked_boost_get<set_result_t>(result.result);
            ok = true;
        } catch (const cannot_perform_query_exc_t &e) {
            error_message = e.what();
            ok = false;
        ...
        try {
            ...
            memcached_protocol_t::write_response_t result;
            rh->nsi->write(write, &result, token, rh->interruptor);
            res = checked_boost_get<append_prepend_result_t>(result.result);
            ok = true;
        } catch (const cannot_perform_query_exc_t &e) {
            error_message = e.what();
            ok = false;
...
void run_incr_decr(txt_memcached_handler_t *rh, pipeliner_acq_t *pipeliner_acq, store_key_t key, uint64_t amount, bool incr, bool noreply, order_token_t token) {
    ...
    try {
        ...
        memcached_protocol_t::write_response_t result;
        rh->nsi->write(write, &result, token, rh->interruptor);
        res = checked_boost_get<incr_decr_result_t>(result.result);
        ok = true;
    } catch (const cannot_perform_query_exc_t &e) {
        error_message = e.what();
...
void run_delete(txt_memcached_handler_t *rh, pipeliner_acq_t *pipeliner_acq, store_key_t key, bool noreply, order_token_t token) {
    ...
    try {
        ...
        memcached_protocol_t::write_response_t result;
        rh->nsi->write(write, &result, token, rh->interruptor);
        res = checked_boost_get<delete_result_t>(result.result);
        ok = true;
    } catch (const cannot_perform_query_exc_t &e) {
        error_message = e.what();
        ok = false;
