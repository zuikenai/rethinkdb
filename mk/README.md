...
* `make clean`: Removed the entire `build/` directory.
...
* `make ALLOW_WRANINGS=1`: Do not fail if the C++ compiler generates a
  warning. Disables `-Werror`
...
* RethinkDB employees can use `./configure CXX=recc`. The executable,
  in `newton:/usr/local/bin/recc`, wraps both `ccache` and `distcc` to
  paralelize and cache the build.
...
* `make build-osx`: Build `dmg` installer for OS X.
